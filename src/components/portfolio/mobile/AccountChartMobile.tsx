...
import { $psShowBalance } from '@/stores/portfolio';
import { $userIsConnected, $userTotalCollateral, $userWethBalance } from '@/stores/user';
...
function AccountChartMobile() {
  ...
  const totalBalance = (wethBalance + totalCollateral).toFixed(4);
      ...
      <div className="flex">
        <div>
          ...
          <div className="mb-[40px] flex">
            ...
            <span className="ml-[6px] text-[20px] font-semibold text-highEmphasis">
              {isConnected ? (!isShowBalance ? '****' : totalCollateral.toFixed(4)) : '-.--'}
            </span>
          </div>
        </div>
        ...
        <div className="ml-[45px]">
          ...
          <div className="flex">
            <Image src="/images/common/symbols/eth-tribe3.svg" alt="" width={20} height={20} />
            <span className="ml-[6px] text-[20px] font-semibold text-highEmphasis">
              {isConnected ? (!isShowBalance ? '****' : wethBalance.toFixed(4)) : '-.--'}
            </span>
          </div>
        </div>
      </div>
